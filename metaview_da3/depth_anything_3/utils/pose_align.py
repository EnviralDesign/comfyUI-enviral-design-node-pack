...
import numpy as np
import torch
# NOTE (MetaView vendoring): `evo` is only needed by align_poses_umeyama (pose
# alignment when input extrinsics are supplied). The MetaView conditioning path
# never passes extrinsics, so this import is deferred into the functions that
# use it to avoid requiring `evo` in the ComfyUI venv.
...
def _umeyama_sim3_from_paths(pose_ref, pose_est):
    from evo.core.trajectory import PosePath3D  # lazy: only when aligning poses
